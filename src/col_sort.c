                            ...
                            void*                     swap_buf)
{
    ...
    {
        swap((uint8_t *)array_base + pivot_idx   * md->elem_size,
             (uint8_t *)array_base + (count - 1) * md->elem_size,
             swap_buf,
             md->elem_size);
    }
    pivot_idx = count - 1;
    ...
    while(left_top != right_top)
    {
        int cmp_res = col_elem_cmp(md,
                                  (uint8_t *)array_base + left_top  * md->elem_size,
                                  (uint8_t *)array_base + pivot_idx * md->elem_size);
        if(cmp_res > 0)
        {
            right_top--;
            swap((uint8_t *)array_base + left_top  * md->elem_size,
                 (uint8_t *)array_base + right_top * md->elem_size,
                 swap_buf,
                 md->elem_size);
        }
        else

...
#define ARRAY_LINEAR_SEARCH_DECL \
    ptrdiff_t COLN_TYPE ## _linear_search(COLN_DATA_TYPE *array, \
                                                size_t array_len, \
                                                COLN_DATA_TYPE *to_find)
#define ARRAY_LINEAR_SEARCH_DEFN \
    ARRAY_LINEAR_SEARCH_DECL \
    { \
        assert(array_len > 0 ? array : true); \
        assert(to_find); \
        for(ptrdiff_t i = 0; i < self->len; i++) \
            if(COLN_DATA_EQUALS(self->data + i, to_find)) return i; \
        return -1; \
    }
#else
#define ARRAY_LINEAR_SEARCH_DECL
...
#ifndef NDEBUG
#define ARRAY_ASSERT_SORTED COLN_TYPE ## _assert_sorted
...
#define ARRAY_ASSERT_SORTED_DEFN \
    ...
    { \
        assert(array_len > 0 ? array : true); \
        for(ptrdiff_t i = 1; i < array_len; i++) \
...
#define ARRAY_BINARY_SEARCH_DECL \
    ptrdiff_t COLN_TYPE ## _binary_search(COLN_DATA_TYPE *array, \
                                          size_t array_len, \
                                          COLN_DATA_TYPE *to_find)
...
#define ARRAY_QUICK_SORT COLN_TYPE ## _quick_sort
